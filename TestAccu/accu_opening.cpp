...
class OpeningTest : public FolderTest
{
public:
	OpeningTest(double width, double height)
	{
		low_ = 100;
		high_ = 200;
		width_ = width;
...
void OpeningTest::run_func()
{
	HRegion Region;
	HRegion Region_dilation;
	int a;
	threshold(GrayImage, Region, low_, high_);
	opening_rectangle1(Region, Region_dilation, width_, height_);
	a = Region_dilation.len;

	//ofstream outfile;
	//outfile.open("C:/Users/02652/Desktop/test/Opencv_RegionOpening.xml", ios::binary | ios::app | ios::in | ios::out, ios::trunc);
	//outfile << "count=" << a << "\n";
	//outfile << "\n";
	//for (int i = 0; i < a; i++){
...
void OpeningTest::prepare_to_validation(int test_case_idx)
{
	/*参数初始化*/
	...
	opening_rectangle1(Region, &RegionDilation, width_, height_);
	...
	//ofstream outfile;
	//outfile.open("C:/Users/02652/Desktop/test/Halcon_RegionOpening.xml", ios::binary | ios::app | ios::in | ios::out, ios::trunc);
...
typedef std::tr1::tuple<double, double> Opening_Low_High_t;
typedef ::testing::TestWithParam<Opening_Low_High_t> Opening_Low_High;
TEST_P(Opening_Low_High, accuracy)
{
	Opening_Low_High_t t = GetParam();
	int width = std::tr1::get<0>(t);
	int height = std::tr1::get<1>(t);
	OpeningTest test(width, height);
	test.safe_run();
}

INSTANTIATE_TEST_CASE_P(Core_Opening, Opening_Low_High, testing::Combine(
