...
class ClosingTest : public FolderTest
{
public:
	ClosingTest(double width, double height)
	{
		low_ = 100;
		high_ = 200;
		width_ = width;
...
void ClosingTest::run_func()
{
	HRegion Region;
	HRegion Region_dilation;
	int a;
	threshold(GrayImage, Region, low_, high_);
	closing_rectangle1(Region, Region_dilation, width_, height_);
	a = Region_dilation.len;

	//ofstream outfile;
	//outfile.open("C:/Users/02652/Desktop/test/Opencv_RegionClosing.xml", ios::binary | ios::app | ios::in | ios::out, ios::trunc);
	//outfile << "count=" << a << "\n";
	//outfile << "\n";
	//for (int i = 0; i < a; i++){
...
void ClosingTest::prepare_to_validation(int test_case_idx)
{
	/*参数初始化*/
	...
	closing_rectangle1(Region, &RegionDilation, width_, height_);
	...
	//ofstream outfile;
	//outfile.open("C:/Users/02652/Desktop/test/Halcon_RegionClosing.xml", ios::binary | ios::app | ios::in | ios::out, ios::trunc);
...
typedef std::tr1::tuple<double, double> Closing_Low_High_t;
typedef ::testing::TestWithParam<Closing_Low_High_t> Closing_Low_High;
TEST_P(Closing_Low_High, accuracy)
{
	Closing_Low_High_t t = GetParam();
	int width = std::tr1::get<0>(t);
	int height = std::tr1::get<1>(t);
	ClosingTest test(width, height);
	test.safe_run();
}

INSTANTIATE_TEST_CASE_P(Core_Opening, Closing_Low_High, testing::Combine(
