...
class ClosingCircleTest : public FolderTest
{
public:
	ClosingCircleTest(double radius_)
	{
		low_ = 100;
		high_ = 200;
		radius = radius_;
...
void ClosingCircleTest::run_func()
{
	HRegion Region;
...
void ClosingCircleTest::prepare_to_validation(int test_case_idx)
{
	/*参数初始化*/
...
typedef std::tr1::tuple<double, double> Closing_Circle_c;
typedef ::testing::TestWithParam<Closing_Circle_c> Closing_Circle_closing;
TEST_P(Closing_Circle_closing, accuracy)
{
	Closing_Circle_c t = GetParam();
	double radius = std::tr1::get<0>(t);
	ClosingCircleTest test(radius);
	test.safe_run();
}

INSTANTIATE_TEST_CASE_P(Core_Erosion_Circle, Closing_Circle_closing, testing::Combine(
	testing::Values(double(9), double(20), double(10), double(12), double(2), double(3), double(25), double(30), double(35)
